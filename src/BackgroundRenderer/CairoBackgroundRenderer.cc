...
void CairoBackgroundRenderer::embed_image(int pageno)
{
    ...
    {
        f_page << (char*)html_renderer->str_fmt("%s/bg%x.svg", param.image_dir.c_str(), pageno);
    }
    f_page << "\"/>";
}
...
{
    // "o" for "PDF Object"
    return string(html_renderer->str_fmt("%s/o%d.jpg", (param.dest_dir + "/" + param.image_dir).c_str(), id));
}
